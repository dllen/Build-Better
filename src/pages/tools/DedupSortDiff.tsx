...
function uniqueSorted(items: string[], opts: { trim: boolean; caseSensitive: boolean; removeEmpty: boolean; sortAsc: boolean }) {
...
export default function DedupSortDiff() {
  ...
  const leftUnique = useMemo(() => uniqueSorted(leftItems, { trim, caseSensitive, removeEmpty, sortAsc }), [leftItems, trim, caseSensitive, removeEmpty, sortAsc]);
  const rightUnique = useMemo(() => uniqueSorted(rightItems, { trim, caseSensitive, removeEmpty, sortAsc }), [rightItems, trim, caseSensitive, removeEmpty, sortAsc]);

  const leftOut = useMemo(() => leftUnique.join("\n"), [leftUnique]);
  const rightOut = useMemo(() => rightUnique.join("\n"), [rightUnique]);

  const parts: DiffPart[] = useMemo(() => JsDiff.diffLines(leftOut, rightOut) as DiffPart[], [leftOut, rightOut]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <textarea className="w-full h-40 rounded-md border border-gray-300 px-3 py-2 font-mono" value={left} onChange={(e) => setLeft(e.target.value)} />
          <div className="flex items-center gap-2">
            <button className="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-md text-sm" onClick={() => copy(leftOut)} disabled={!leftOut}>
              <ClipboardCopy className="h-4 w-4" /> Copy Dedup+Sorted {copied ? <Check className="h-4 w-4 text-green-600" /> : null}
            </button>
          </div>
          <div className="rounded-md border border-gray-200 p-3">
            <pre className="whitespace-pre-wrap break-words text-sm font-mono">{leftOut || "—"}</pre>
          </div>
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-700">Right</label>
          <textarea className="w-full h-40 rounded-md border border-gray-300 px-3 py-2 font-mono" value={right} onChange={(e) => setRight(e.target.value)} />
          <div className="flex items-center gap-2">
            <button className="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-md text-sm" onClick={() => copy(rightOut)} disabled={!rightOut}>
              <ClipboardCopy className="h-4 w-4" /> Copy Dedup+Sorted {copied ? <Check className="h-4 w-4 text-green-600" /> : null}
            </button>
          </div>
          <div className="rounded-md border border-gray-200 p-3">
            <pre className="whitespace-pre-wrap break-words text-sm font-mono">{rightOut || "—"}</pre>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select className="rounded-md border border-gray-300 px-3 py-2" value={String(sortAsc)} onChange={(e) => setSortAsc(e.target.value === "true")}>
              <option value="true">升序</option>
              <option value="false">降序</option>
            </select>
          </div>
        </div>
        <div className="text-xs text-gray-600">左唯一: {stats.leftUnique}，右唯一: {stats.rightUnique}，交集: {stats.intersection}，diff added: {stats.added}，diff removed: {stats.removed}</div>
      </div>
