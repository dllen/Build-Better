...
type Language = "javascript" | "typescript" | "json" | "html" | "css" | "markdown" | "yaml" | "graphql";
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-6 px-4">
      <SEO 
        ...
        keywords={["code formatter", "prettier online", "json formatter", "javascript formatter", "代码格式化", "美化代码"]}
      />
          ...
          <div className="flex items-center gap-2 bg-white border border-gray-300 rounded-lg px-3 py-1.5 shadow-sm">
            ...
            >
              {Object.entries(THEMES).map(([value, labelKey]) => (
                <option key={value} value={value}>{t(`tools.code-formatter.themes.${labelKey}`)}</option>
              ))}
            </select>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-250px)] min-h-[500px]">
        ...
        <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50/50">
            <label className="text-sm font-semibold text-gray-700">{t('tools.code-formatter.input_label')}</label>
            <span className="text-xs text-gray-400 font-mono">{t('tools.code-formatter.input_placeholder')}</span>
          </div>
        ...
        <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 bg-gray-50/50">
            <label className="text-sm font-semibold text-gray-700">{t('tools.code-formatter.output_label')}</label>
            <button
              onClick={handleCopy}
              disabled={!output}
            ...
            {error ? (
              <div className="absolute inset-0 p-6 bg-red-50/50">
                <div className="bg-white p-6 rounded-xl border border-red-200 shadow-sm">
                  <h3 className="text-red-600 font-semibold mb-2">{t('tools.code-formatter.error_title')}</h3>
