...
export default function TextDeduper() {
  ...
  const stats = useMemo(() => {
    const input = items.filter(
      (x) => !(removeEmpty && normalize(x, { trim, caseSensitive }).length === 0)
    ).length;
  ...
  const uniqueText = useMemo(
    () => uniqueList.map((x) => x.value).join(outDelimiter),
    [uniqueList, outDelimiter]
  );
  const countsText = useMemo(
    () => uniqueList.map((x) => `${x.value}\t${x.count}`).join("\n"),
    [uniqueList]
  );
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <select
              ...
              disabled={keepOrder || sortMode === "none"}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input
              ...
              placeholder="输出分隔符，如换行或逗号"
              value={outDelimiter}
              onChange={(e) => setOutDelimiter(e.target.value)}
            />
            <button
              className="inline-flex items-center gap-2 px-3 py-2 bg-amber-600 text-white rounded-md text-sm hover:bg-amber-700"
              onClick={() => copy(uniqueText)}
              disabled={!uniqueText}
            >
              <ClipboardCopy className="h-4 w-4" /> 复制去重文本{" "}
