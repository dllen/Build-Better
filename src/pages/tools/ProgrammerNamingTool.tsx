...
const verbs = ["get","set","create","update","delete","remove","list","find","fetch","load","save","compute","calculate","convert","parse","format","validate","normalize","merge","split","diff","map","reduce","filter","sort","aggregate","encode","decode","encrypt","decrypt","hash","sign","verify","generate","build"];
const nouns = ["data","cache","manager","service","client","server","controller","handler","adapter","processor","generator","builder","factory","repository","store","queue","task","job","result","response","request","config","logger","monitor","worker","session","token","util"];
const qualifiers = ["async","sync","local","remote","global","temp","main","core","default","primary"];
...
function buildSuggestions(kind: Kind, style: Style, keywords: string[], prefix: string, suffix: string, randomness: number, count: number) {
  const list: string[] = [];
  for (let i = 0; i < count; i++) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
          ...
          <input className="w-full rounded-md border border-gray-300 px-3 py-2" placeholder="输入关键词，如 user token auth" value={input} onChange={(e) => setInput(e.target.value)} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-sm text-gray-700">类型</label>
              <select className="rounded-md border border-gray-300 px-3 py-2" value={kind} onChange={(e) => { const k = e.target.value as Kind; setKind(k); setStyle(kindDefaultStyle(k)); }}>
            ...
            <div className="space-y-1">
              ...
              <select className="rounded-md border border-gray-300 px-3 py-2" value={style} onChange={(e) => setStyle(e.target.value as Style)}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input className="rounded-md border border-gray-300 px-3 py-2" placeholder="前缀（可选）" value={prefix} onChange={(e) => setPrefix(e.target.value)} />
            <input className="rounded-md border border-gray-300 px-3 py-2" placeholder="后缀（可选）" value={suffix} onChange={(e) => setSuffix(e.target.value)} />
            <div className="space-y-1">
              <label className="text-sm text-gray-700">随机性: {Math.round(randomness * 100)}%</label>
              <input type="range" min={0} max={1} step={0.01} value={randomness} onChange={(e) => setRandomness(parseFloat(e.target.value))} />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-sm text-gray-700">数量: {count}</label>
              <input type="range" min={4} max={48} step={1} value={count} onChange={(e) => setCount(parseInt(e.target.value))} />
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {suggestions.map((s, i) => (
              <div key={i} className="flex items-center gap-2 rounded-md border border-gray-200 px-3 py-2">
                ...
                <button className="inline-flex items-center gap-2 px-2 py-1 border border-gray-300 rounded-md text-xs" onClick={() => copy(s, "one", i)}>
                  <ClipboardCopy className="h-4 w-4" /> 复制 {copiedOne === i ? <Check className="h-4 w-4 text-green-600" /> : null}
                </button>
              </div>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <button className="inline-flex items-center gap-2 px-3 py-2 border border-gray-300 rounded-md text-sm" onClick={() => copy(suggestions.join("\n"), "all")} disabled={!suggestions.length}>
              <ClipboardCopy className="h-4 w-4" /> 复制全部 {copiedAll ? <Check className="h-4 w-4 text-green-600" /> : null}
            </button>
          </div>
        </div>
      </div>
